% LIVING SOURCE — this is the only resume .tex to keep editing.
% Former filename: HarshDesai_Resume_KLA_OptoMechanical.tex
% PDFs from this file live in pdfs-from-living-source/. Never overwrite an existing PDF.
...
\textbf{Mechanical System Design \& CAE:} ANSYS (GUI/TUI): Fluent, Mechanical, SolidWorks, Fusion 360, AutoCAD, Autodesk Inventor, SpaceClaim, conceptual design, 3D part/assembly modeling\\[-0.2pt]
\textbf{Instrumentation \& Automation:} sensors and detector systems, electrostatic ion optics, GPIB/IEEE-488, serial data logging, NI VISA (PyVISA, PyMeasure, PySerial), DAQs and SMUs, high-voltage switching operations, vacuum chamber operation (CTI-Cryogenics Hi-Vac and roughing pump, operated at 1~mTorr)\\[-0.2pt]
% \textbf{Plasma \& Diagnostics:} Low-pressure hollow-cathode plasma operation, 1.3~kV HV interface validation, RPA, emissive-probe and Langmuir-probe diagnostics, CEM/ESA calibration\\[-0.2pt]
\textbf{Languages:} Python, LabVIEW (CLAD prep), SCPI, MATLAB (learning), Verilog (learning)\\[-0.2pt]
\textbf{FPGA, Ion Optics \& Modeling Tools:} FPGA design workflow, AMD Vivado, MATLAB HDL Coder and DSP Toolbox, SIMION, SRIM, Amptek DPPMCA\\[0.2pt]
\textbf{Documentation \& Analysis:} Microsoft Excel, Word, PowerPoint, LaTeX, Google Data/Looker Studio; Testing Plans and Procedures, FMEA, requirements and verification documentation, Systems Engineering Workflow
