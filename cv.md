...
- **Instrumentation & DAQ / Hardware:** GPIB/IEEE-488, USB-serial, serial data logging, NI VISA (PyVISA, PyMeasure, PySerial), Keithley DAQs and SMUs, TDK Lambda power supplies (PSUs), high-voltage switching operations, vacuum chamber operation (CTI-Cryogenics Hi-Vac and roughing pump, operated at 1 milliTorr)
- **FPGA, Plasma & Modeling Tools:** FPGA design workflow, SIMION, SRIM, Amptek DPPMCA, AMD Vivado, MATLAB HDL Coder and DSP Toolbox, SPENVIS, Space Weather Modeling Framework (SWMF), Magnetosphere-Ionosphere models (MAGE, SWMF, HYPERS), Google Data Studio, QGIS
...
- Validated a 1300 V hollow-cathode plasma-source test workflow by combining high-voltage discharge-box failure mode and effects analysis (FMEA), Python/PyVISA ignition control, power-supply logging, and DAQ synchronization to protect hardware during high-throughput characterization.
...
- Characterized remote ignition and post-run processing workflows for Spacecraft Charging Device test readiness, reaching 98.6% synchronized logging coverage across data streams, improving experimental repeatability, and saving approximately 15 minutes of post-run processing per run.
...
- Ran Uranus cloud-resolving simulations on the Great Lakes High-Performance Computing (HPC) cluster to study methane abundance/profile sensitivity, latent heating, microphysics settings, and gravity.
...
- Developed a digital readout roadmap for solid-state detectors by sizing ADC, sampling-rate, and digital-shaping requirements against science-driven particle-detection energy-resolution targets.
...
- Developed "TestBedz", a full-stack requirement-flowdown platform for spacecraft qualification testing that collapses multi-round requirement negotiations into structured submissions for thermal-vacuum (TVAC), vibration, and electromagnetic interference (EMI) testing cycles.
- Implemented a spacecraft environmental-test feasibility and routing system for SPRL, integrating hardware profiles, facility capability models, test-profile constraints, and requirements into automated, contract-ready test plans.
...
- Led communications subsystem architecture trades for the Uranian Orbiter and Probe mission study, managing 15 Deep Space Network (DSN) connectivity and fault-tolerance requirements through a requirements traceability matrix.
...
**Dates:** Jan'25--Apr'25
**Report:** [Link](https://drive.google.com/file/d/1x6fm2dZtLi4hjcGfQf2WpbNDZLsGMDGT/view?usp=sharing) | **Course:** SPACE 477 | **Advisor:** Prof. Xianzhe Jia
...
### Spaceport America Cup / Intercollegiate Rocket Engineering Competition (IREC)

**Dates:** Apr'20--Jul'21
**Report:** [Link](https://drive.google.com/file/d/19GcuGf8MkgRR0t0MPE-XhEvW5EahuQBZ/view?usp=drive_link)

- Launched a Mach 0.9 solid-motor rocket to a 10,000-foot target altitude; team placed 23rd globally and 5th in Asia-Pacific.
- Developed a student-researched and developed (SRAD) trajectory simulator using computational fluid dynamics (CFD)-backed aerodynamic data for flight prediction.
...
- Modeled maple-seed mono-wing payload descent using Blade Element Theory and transient computational fluid dynamics (CFD), with MATLAB telemetry software for real-time dual-payload deployment monitoring.
...
- Built a payload release and camera-stabilization stack: elastic-lid release with torsion-spring lock for parachute deployment, plus a dual-servo 2-axis gimbal maintaining a fixed 45 degrees downward south-facing camera vector.
