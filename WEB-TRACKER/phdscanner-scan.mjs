...
import { fetchPhdscannerPostings } from './lib/phdscanner/source-adapter.mjs';
import { normalizePhdscannerPosting } from './lib/phdscanner/normalizer.mjs';
import {
  loadRegistry,
  loadState,
  saveState,
...
const LOCK_FILE = join(RUNTIME_DIR, 'phdscanner-scan.lock');
...
function acquireLock() {
  ...
  if (existsSync(LOCK_FILE)) {
    ...
    if (Number.isFinite(ageMs) && ageMs < LOCK_TTL_MS) {
      throw new Error('PhDScanner scan already running; lock file is fresh.');
    }
  }
...
function queueDiscoveryEvent(source, opportunities) {
  ...
  appendFileSync(EVENT_QUEUE, `${JSON.stringify({
    type: 'new_phdscanner_opportunities',
    source_id: source.id,
    source: source.id,
...
function refreshDeadlineStatuses(now = new Date()) {
  const existing = readPhdscannerOpportunities();
  let changed = 0;
  ...
  if (!DRY_RUN && changed > 0) {
    writePhdscannerOpportunities({ ...existing, opportunities });
    syncPhdscannerOpportunitiesToDashboard();
  }
  return changed;
}
...
async function scanSource(source, state, registry) {
  ...
  const result = await fetchPhdscannerPostings(sourceWithCaps);
  const status = result.status === 'ok' ? (result.http_status || 200) : result.status === 'blocked' ? 451 : 0;
  const opportunities = (result.postings || []).map(posting => normalizePhdscannerPosting(posting, {
    sourceId: source.id,
    now,
  }));
  const scanSummary = {
  ...
  const { store, newOpportunities } = DRY_RUN
    ...
    : mergePhdscannerOpportunities(opportunities, { scanSummary });
...
try {
  const phdscannerSources = registry.sources.filter(source =>
    source.enabled && source.platform === 'phdscanner'
  );
  let toScan;
  if (SOURCE_FILTER) {
    toScan = phdscannerSources.filter(source => source.id === SOURCE_FILTER);
  } else if (SCAN_ALL) {
    toScan = phdscannerSources;
  } else {
    const due = getDueSources(registry, state);
    toScan = due.filter(source => source.platform === 'phdscanner');
  }

  if (REFRESH_LIVENESS) {
    const refreshed = refreshDeadlineStatuses();
    console.log(`[phdscanner-scan] Refreshed ${refreshed} expired local PhDScanner prospect(s).`);
  }

  console.log(`\n[phdscanner-scan] ${DRY_RUN ? '(DRY RUN) ' : ''}Scanning ${toScan.length} PhDScanner source(s)...\n`);
  const results = [];
  for (const source of toScan) {
    const result = await scanSource(source, state, registry).catch(err => ({
      source_id: source.id,
      status: 'failed',
      error: err.message,
    }));
  ...
  if (!DRY_RUN && (RESCORE || !toScan.length)) {
    const rescored = rescorePhdscannerOpportunities();
    syncPhdscannerOpportunitiesToDashboard();
    console.log(`[phdscanner-scan] Rescored ${rescored.opportunities?.length || 0} opportunities (visible ${rescored.scan_summary?.visible_count || 0}, archived ${rescored.scan_summary?.archived_count || 0}).`);
  }
