...
import { processPhdscannerFactory, RUNTIME_DIR } from './lib/phdscanner/factory.mjs';

const BASE = dirname(fileURLToPath(import.meta.url));
const LOCK_FILE = join(RUNTIME_DIR, 'phdscanner-factory-worker.lock');
...
async function main() {
  ...
  if (!dryRun && !acquireLock()) {
    return {
      ok: true,
      skipped: true,
      reason: 'phdscanner factory worker already running',
      lock_file: LOCK_FILE,
    };
  }
  try {
