...
import { processExhibitorFactory, RUNTIME_DIR } from './lib/exhibitor/factory.mjs';

const BASE = dirname(fileURLToPath(import.meta.url));
const LOCK_FILE = join(RUNTIME_DIR, 'exhibitor-factory.lock');
...
async function main() {
  const max = Number(argValue('--max', 20));
  ...
  if (!dryRun && !acquireLock()) {
    return {
      ok: true,
      skipped: true,
      reason: 'exhibitor factory worker already running',
      lock_file: LOCK_FILE,
    };
  }
  try {
    if (dryRun) {
      return {
        ok: true,
        dry_run: true,
        ...(await processExhibitorFactory({
          max: Number.isFinite(max) && max > 0 ? max : 20,
          force,
        })),
      };
    }
    const result = await processExhibitorFactory({
      max: Number.isFinite(max) && max > 0 ? max : 20,
      force,
    });
