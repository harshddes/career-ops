...
/** Core instrumentation / plasma / space / nuclear / detector hits. */
...
/**
 * Engineering-adjacent domains. These keep EE / quantum / materials postings
 * visible even when plasma keywords are absent.
 */
...
/** Undesired as daily work unless a substantial physical hardware anchor exists. */
...
/**
 * Weak singles need a real domain anchor (strong term or non-weak adjacent).
 * Role alone must NOT unlock these — otherwise every "Laboratory of X" floats into Adjacent.
 */
...
/** Roles Harsh can pursue (PhD / engineer / research support). */
const ROLE_POSITIVE = [
...
/**
 * Faculty / postdoc titles Harsh is not targeting → hard archive.
 * Match as whole phrases where possible to avoid "Prof. Dr." supervision noise in PhD ads.
 */
const ROLE_ARCHIVE = [
...
/**
 * Topic junk. Prefer specific bio/clinical terms over broad "health" —
 * MedTech manufacturing PhDs must stay visible.
 */
const NEGATIVE = [
...
/** Prefer whole-token matches for short/ambiguous terms (control≠controlled, lip≠multiple). */
...
/**
 * "Prof. Dr. X supervises" in a PhD ad is not a faculty job.
 * Only treat professor hits as archive when they look like the job title itself.
 */
function roleArchiveMatchesFor(posting = {}, text = '') {
  ...
  // Body-only "postdoctoral" / "assistant professor" still means wrong role class.
  const bodyHits = unique(includesAny(text, [
...
export function scoreEuraxessPosting(posting = {}, now = new Date()) {
  ...
  if (strongMatches.some(term => ['space', 'space science', 'space plasma', 'spacecraft', 'cern', 'cryogenic', 'cryogenics', 'synchrotron', 'nuclear', 'fusion'].includes(term))) {
    score += 0.15;
  }
