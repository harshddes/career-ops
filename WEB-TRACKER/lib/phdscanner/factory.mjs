...
/** Dedicated PhDScanner lane â€” never mix with Jobs-to-Consider / Helion / CFS agent-tasks.ndjson */
export const TASKS_FILE = join(DATA_DIR, 'phdscanner-agent-tasks.ndjson');
export const LEGACY_SHARED_TASKS_FILE = join(DATA_DIR, 'agent-tasks.ndjson');
export const FACTORY_RUN_DIR = join(RUNTIME_DIR, 'phdscanner-factory');
...
export async function phdscannerFactoryRunnerHealth({ env = process.env } = {}) {
  ...
  const artifactsEnabled = ['1', 'true', 'yes', 'on'].includes(cleanText(env.PHDSCANNER_FACTORY_ARTIFACTS_ENABLED).toLowerCase());
  return {
    ...
    artifact_runner: {
      ...
      command: cleanText(env.PHDSCANNER_FACTORY_ARTIFACT_RUNNER || 'codex'),
      ok: artifactsEnabled && codex.ok,
      detail: codex.stdout || codex.stderr || codex.error || (artifactsEnabled ? '' : 'Set PHDSCANNER_FACTORY_ARTIFACTS_ENABLED=true to allow draft application-pack generation.'),
    },
  };
}

function opportunityCompany(opportunity = {}) {
  return opportunity.university || opportunity.institution || 'PhDScanner';
}
...
export function ensurePhdscannerConsiderJob(opportunity = {}) {
  ...
  upsertConsiderJob({
    ...
    notes: [
      `Queued from PhDScanner Factory (${Number(opportunity.score || 0).toFixed(1)}/5).`,
...
export function createFactoryTasks(opportunity, jobsToConsiderId) {
  ...
  const researchTask = {
    ...base,
    type: 'deep_research',
    notes: `PhDScanner Factory score ${Number(opportunity.score || 0).toFixed(1)}/5. Verify deadline/application status first. ${opportunity.fit_rationale || ''}`,
  };
  ...
  if (Number(opportunity.score || 0) >= PACK_THRESHOLD) {
    const artifactTask = {
      ...
      notes: 'Create draft artifacts only after confirming the PhDScanner posting is still active. Never submit automatically.',
    };
...
export function queuePhdscannerOpportunityWork(opportunity = {}, { pack = false } = {}) {
  ...
  if (pack) {
    const artifactTask = {
      ...
      artifact_kind: 'application_pack',
      expected_resources: ['resume_tex', 'resume_pdf', 'cover_letter_pdf', 'email_draft'],
      notes: 'Queued from PhDScanner dashboard. Draft only; never submit automatically.',
    };
    ...
    const researchTask = {
      ...base,
      type: 'deep_research',
      notes: `Queued from PhDScanner dashboard. Score ${Number(opportunity.score || 0).toFixed(1)}/5. ${opportunity.fit_rationale || ''}`,
    };
...
function researchTopic(opportunity = {}) {
  return [
    'Research this PhDScanner opportunity using current public information.',
    `Title: ${opportunity.title}`,
...
function reportPathFor(opportunity = {}) {
  ...
  return join(CAREER_OPS_DIR, 'reports', `phdscanner-${slug}-${today()}.md`);
}
...
function buildReport({ opportunity, researchText = '', researchResultUrl = '' }) {
  return [
    `# PhDScanner Research: ${opportunity.title}`,
    '',
    `**Institution:** ${opportunity.university || opportunity.institution || 'PhDScanner'}`,
    `**URL:** ${opportunity.url}`,
    `**Score:** ${Number(opportunity.score || 0).toFixed(1)}/5`,
    ...
    '## Verification First',
    '',
    '- Verify deadline and application status on the PhDScanner/employer page before using any draft artifacts.',
...
export function selectFactoryCandidates(store = readPhdscannerOpportunities(), { max = 3, force = false, retryFailures = false } = {}) {
...
export async function processPhdscannerFactory({ max = 3, dryRun = false, force = false, retryFailures = false, pollTimeoutSec = 120 } = {}) {
  ...
  for (const opportunity of candidates) {
    ...
    if (research.status === 'completed') {
      ...
      ensurePhdscannerConsiderJob({ ...opportunity, ...patch, jobs_to_consider_id: jobsToConsiderId });
    ...
    const updated = patchPhdscannerOpportunity(opportunity.id, patch).opportunity;
    results.push({
      id: opportunity.id,
      title: opportunity.title,
      stage,
  ...
  if (!dryRun) syncPhdscannerOpportunitiesToDashboard();
  return {
    generated_at: new Date().toISOString(),
...
export async function phdscannerFactoryStatus() {
  const store = readPhdscannerOpportunities();
  const health = await phdscannerFactoryRunnerHealth();
