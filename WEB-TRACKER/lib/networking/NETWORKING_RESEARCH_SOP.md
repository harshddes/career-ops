...
4. Deduplicate against existing people via email / LinkedIn URL / GitHub URL before creating a new record. Ambiguous name+org matches stay in duplicate review.
5. Upsert organization, then upsert people with:
...
6. Call `markNetworkingResearchReviewReady(order.id, candidatePersonIds)`.
7. After the user can see candidates on the Networking tab, call `completeNetworkingResearch(order.id)` and `syncNetworkingToDashboard()`.
